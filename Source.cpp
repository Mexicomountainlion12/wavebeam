	...
	while (!doexit)//game loop!
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (key[KEY_SPACE]) {
				...
				BeamX += 30;
				BeamX = 50 * cos(1*alpha+0)+xPos+24;
				BeamY = 50 * sin(1 * alpha + 0) + yPos +30;
				BeamX2 += 10;
				BeamY2 = 200 * sin(1* alpha + 0) + yPos + 8;
				isShooting = true;
			}
			else {
				isShooting = false;
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			if (isShooting) {
				al_draw_filled_circle(BeamX, BeamY, 9, al_map_rgb(0, 255, 200)); //draw bullet
				al_draw_filled_circle(BeamX2, BeamY2, 9, al_map_rgb(100, 200, 0));
			}
